...
- **Total if using Merlin**: $0.05 (L2) + $0.65 (bridge out) = **$0.70** (slightly more expensive)
...
| **Highest cost component** | Dust limit (~$0.30, or 46% of total) |
...
│                                                                 │
│   REAL WORLD ANALOGY:                                           │
│   └── Imagine if the government said: "Bills under $0.30       │
│       are too small to be worth handling"                       │
│   └── You couldn't create or transfer $0.15 bills              │
│   └── The minimum "bill" would be $0.30                        │
│                                                                 │
...
**🔴 Highest Cost Component: Dust Limit (46%)** - At minimum fee rates, the fixed dust limit becomes the largest cost component. This cannot be reduced—it's the minimum "bill size" the network accepts.

#### Cost at Different Fee Rates

| Fee Rate | Inscribe TX | Send TX | Dust Limit | Total | Highest Cost |
|----------|-------------|---------|------------|-------|--------------|
| 1 sat/vbyte | $0.17 | $0.18 | $0.30 | **$0.65** | Dust (46%) |
| 10 sat/vbyte | $1.67 | $1.83 | $0.30 | $3.80 | Send TX |
| 30 sat/vbyte | $5.01 | $5.50 | $0.30 | $10.81 | Send TX |
| 60 sat/vbyte | $10.02 | $11.00 | $0.30 | $21.32 | Send TX |
...
*Note: ~46% of the cost (~$0.30) is the dust limit, which is fixed regardless of fee rate. Costs vary with BTC price.*
...
| L1 Inscription TX | ~$0.17 | **Beem** (bridge operator) | ✅ L1 |
| L1 Send TX | ~$0.18 | **Beem** (bridge operator) | ✅ L1 |
| Dust Limit (330 sats) | ~$0.30 | Beem | ✅ L1 |
| **TOTAL per user** | **~$0.70** | | |

**Key Insight**: At 1 sat/vbyte, the dust limit (~$0.30) represents 46% of the bridge-out cost.

### Who Pays Bridge-Out Costs?

| Model | Description | Cost to Beem per User |
|-------|-------------|----------------------|
| **Beem Pays All** | Beem covers all withdrawal costs | ~$0.70 |
...
|----------|-------------------|-----------------|-----------|
| **Pure L1** | ~$650K | $0 (already on L1) | **~$650K** |
| **L2 + Bridge Out (Beem pays)** | $50K | ~$700K | **~$750K** |
| **L2 + Bridge Out (User pays)** | $50K | $0 | **$50K** |
...
│   • Inscription TX (~$0.17) - write data to Bitcoin            │
│   • Send TX (~$0.18) - transfer UTXO to user                   │
...
|------|-------------|------|----------|
| 1 | Inscribe "transfer X BID" | ~$0.17 | |
| 2 | Send UTXO to user's Taproot | ~$0.18 | |
| 3 | Dust locked in UTXO | ~$0.30 | 🔴 **YES (46%)** |
| **TOTAL** | | **~$0.65** | |
...
| 3 | Beem inscribes "transfer X BID" | ~$0.17 | |
| 4 | Beem sends UTXO to user | ~$0.18 | |
| 5 | Dust locked in UTXO | ~$0.30 | 🔴 **YES (46%)** |
| **TOTAL** | | **~$0.65** | |
...
| 4 | Bridge inscribes "transfer X BID" | ~$0.17 | |
| 5 | Bridge sends UTXO to user | ~$0.18 | |
| 6 | Dust locked in UTXO | ~$0.30 | 🔴 **YES (46%)** |
| **TOTAL (per user)** | | **~$0.75** | |

### Key Insight (@ 1 sat/vbyte)

**At minimum fee rates, the dust limit (~$0.30) becomes the highest cost component (~46%).**
...
| L2 → L1 | $50K | ~$650K | **~$700K** | **~$0.70** |

**Conclusion**: At 1 sat/vbyte, all approaches cost ~$650-700K for 1 million users if Beem pays everything.
...
| Shadow Balance | 100K get L1 | 900K stay shadow | **~$65K** |
| L2 | 100K bridge out | 900K stay L2 | **~$120K** |
...
4. **Beem covers only dust** ($0.30/user) or passes that to user too
...
|---------------|----------------------|----------------------|
| Minimum | 1 | ~$0.65 |
| Very Low | 1-5 | ~$0.65-2.10 |
| Low | 5-15 | ~$2.10-5.60 |
| Normal | 15-30 | ~$5.60-10.80 |
| High | 30-60 | ~$10.80-21.30 |
| Very High | 60-100+ | ~$21.30-35+ |

*Note: We typically use 1 sat/vbyte for transactions to minimize costs.*

### Key Takeaways

1. **Every real BRC-20 transfer needs L1 inscription** (~$0.17 at 1 sat/vbyte, ~$10-11 at 30 sat/vbyte)
2. **Total L1 cost at 1 sat/vbyte: ~$0.65 per user** (inscription + send + dust)
3. **At 1 sat/vbyte, dust limit (~$0.30) is 46% of total cost** - this is fixed and cannot be reduced
