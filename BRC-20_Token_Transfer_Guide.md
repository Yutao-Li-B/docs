...
**Answer**: **~$0.94 per user** — there is no cheaper way.

---

## Why ~$0.94? (The Dollar Bill Analogy)
...
│   → Delivery/postage costs ~$0.44 (transaction fees)           │
│   → TOTAL: ~$0.94 per transfer                                 │
│                                                                 │
...
| **Real BRC-20 (L1)** | **~$0.94** | ✅ ON Bitcoin. Visible on Unisat. User actually owns it. |
...
- To convert to real BRC-20: user must **bridge out** → costs **~$0.94**
- **Total if using Merlin**: $0.05 (L2) + $0.94 (bridge out) = **$0.99** (more expensive!)
...
│   → Pay ~$0.94 to retrieve from vault (bridge out to L1)       │
...
|------|---------------|-------|
| Real BRC-20 on Bitcoin (visible on Unisat) | **~$0.94** | No way to reduce this |
...
**Cost breakdown at 1 sat/vbyte (lowest rate):**
- Inscription transaction: ~$0.28
- Send transaction: ~$0.17  
- Dust limit (minimum UTXO): ~$0.50 ← **Fixed, cannot reduce**
- **Total: ~$0.94**
...
| **BRC-20 transfer cost (L1)** | **~$0.94 per transfer** |
...
|----------|---------------|----------------|--------------|----------------------|
| **L1 Direct** | ~$0.94 | ✅ Yes | ✅ Yes | Already on L1 |
| **Shadow Balance** | $0 | ❌ No | ❌ No (database) | Redeem: +$0.94 |
| **Merlin L2** | ~$0.05 | ❌ No (wrapped) | ❌ No (M-Token) | Bridge out: +$0.94 |
| **Spark (LRC-20)** | ~$0.01-0.05 | ❌ No | ❌ No (different token) | ❌ Cannot convert |

**Bottom Line**: The ~$0.94 L1 inscription cost is unavoidable if you want real, verifiable BRC-20 tokens on the Bitcoin mainnet.
...
#### Cost Breakdown at 1 sat/vbyte

| Component | Calculation | Sats | USD | % of Total |
|-----------|-------------|------|-----|------------|
| Inscribe TX | 300 vbytes × 1 sat | 300 | $0.28 | 27% |
| Send TX | 180 vbytes × 1 sat | 180 | $0.17 | 16% |
| Dust Limit | Fixed (SegWit) | 546 | $0.50 | **53%** |
| **TOTAL** | | **1,026** | **$0.94** | 100% |

**🔴 Highest Cost Component: Dust Limit (53%)** - At minimum fee rates, the fixed dust limit becomes the largest cost component. This cannot be reduced—it's the minimum "bill size" the network accepts.

#### Cost at Different Fee Rates

| Fee Rate | Inscribe TX | Send TX | Dust Limit | Total | Highest Cost |
|----------|-------------|---------|------------|-------|--------------|
| 1 sat/vbyte | $0.28 | $0.17 | $0.50 | **$0.94** | Dust (53%) |
| 10 sat/vbyte | $2.75 | $1.65 | $0.50 | $4.91 | Inscribe TX |
| 30 sat/vbyte | $8.26 | $4.96 | $0.50 | $13.72 | Inscribe TX |
| 60 sat/vbyte | $16.52 | $9.91 | $0.50 | $26.93 | Inscribe TX |
...
| **Transfer BRC-20 token** | **~$0.94** | Per transfer (2 TXs + dust) |
...
```
┌─────────────────────────────────────────────────────────────────┐
│     EVERY PATH TO REAL L1 BRC-20 COSTS ~$0.94                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   FROM SHADOW BALANCE:                                          │
│   ┌──────────────┐         ┌──────────────┐                    │
...
│   DIRECT L1:                                                    │
│   ┌──────────────┐         ┌──────────────┐                    │
│   │ Your L1      │ ──$0.94──▶ │ User's L1   │                    │
│   │ BRC-20       │ transfer │ BRC-20      │                    │
│   └──────────────┘         │ (on-chain)  │                    │
│                            └──────────────┘                    │
│                                                                 │
│   THE ~$0.94 L1 COST IS UNAVOIDABLE because:                   │
...
|----------|-----------------|-------------|-------------------------|----------------|----------|
| **BRC-20** | ~$0.94 | ✅ Witness | ✅ Unisat, OKX, OrdScan | L1 Ordinals indexers | ⭐⭐⭐⭐⭐ High |
| **BRC2.0** | ~$0.94 | ✅ Witness | ✅ Unisat + BRC2.0 indexer | L1 Ordinals + BRC2.0 indexers | ⭐⭐⭐ Growing |
...
| **ORC-20** | ~$0.94 | ✅ Witness | ⚠️ ORC-20 explorers | L1 ORC-20 indexers | ⭐⭐ Low |
...
| Pros | Cons |
|------|------|
| ✅ Highest adoption | ❌ ~$0.94/transfer @ 1 sat/vB (higher at congestion) |
...
| **Transfer cost @ 1 sat/vB** | ~$0.94 | ~$0.40 |
...
| **Bridge-out cost** | ~$0.94 (L1 inscription + send + dust) |
...
| Cheapest L1 fungible tokens | **Runes** | ~$0.40 vs BRC-20's ~$0.94 | ✅ Yes |
...
|----------|-----------------|-------|---------------------|-----------------------------|--------------------|
| **BRC-20** | ~$0.94 | L1 | ✅ Yes (witness) | ✅ Yes | ✅ Native |
| **BRC2.0** | ~$0.94 | L1 | ✅ Yes (witness) | ✅ Yes (+ BRC2.0 indexer) | ✅ Extension |
| **Runes** | ~$0.40 | L1 | ✅ Yes (OP_RETURN) | ✅ Yes (Runes explorers) | ❌ Different |
| **ORC-20** | ~$0.94 | L1 | ✅ Yes (witness) | ⚠️ ORC-20 explorers | ❌ Different |
...
- **Cheapest with eventual L1 visibility**: Merlin L2 (~$0.05 on L2, ~$0.94 to bridge out)
...
|----------|--------------|------------|----------------|--------------|
| Pure L1 | ~$940K | - | **~$940K** | **~$0.94** |
| Shadow → L1 | $0 | ~$940K | **~$940K** | **~$0.94** |
| L2 → L1 | $50K | ~$940K | **~$990K** | **~$0.99** |
...
|----------|-------------------|---------------|-------------------|
| Pure L1 | All users get L1 | N/A | **~$940K** |
...
│    ├── YES → Pure L1 (cost ~$0.94/user @ 1 sat/vB)             │
│    └── NO → Continue to Q2                                     │
│                                                                 │
...
|----------|---------------------|-----------|-----------------|
| Pure L1, Beem pays | ~$940K | $0 | ✅ All users |
| Shadow + User-paid L1 | ~$0 | ~$0.94 at redemption | ✅ On demand |
| L2 + User-paid L1 | ~$50K | ~$0.94 at bridge-out | ✅ On demand |
...
3. **Offer L1 redemption** where **user pays the L1 fees** (~$0.94 at 1 sat/vbyte)
4. **Beem covers only dust** ($0.50/user) or passes that to user too

**Total Beem cost for 1M users: $0 - $50K** (vs ~$940K for pure L1 at 1 sat/vbyte)
...
┌─────────────────────────────────────────────────────────────────┐
│                                                                 │
│   🔴 TO GET REAL BRC-20 ON L1: ~$0.94 per user                 │
│                                                                 │
...
│   there is NO way to avoid the ~$0.94 L1 cost.                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
2. **Total L1 cost at 1 sat/vbyte: ~$0.94 per user** (inscription + send + dust)
