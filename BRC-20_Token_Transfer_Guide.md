...
**Answer**: **~$0.85 per user** — there is no cheaper way.

---

## Why ~$0.85? (The Dollar Bill Analogy)
...
│   → The bill itself costs ~$0.50 (minimum Bitcoin requires)    │
│   → Delivery/postage costs ~$0.35 (transaction fees)           │
│   → TOTAL: ~$0.85 per transfer                                 │
│                                                                 │
...
| **Real BRC-20 (L1)** | **~$0.85** | ✅ ON Bitcoin. Visible on Unisat. User actually owns it. |
...
- To convert to real BRC-20: user must **bridge out** → costs **~$0.85**
- **Total if using Merlin**: $0.05 (L2) + $0.85 (bridge out) = **$0.99** (more expensive!)
...
│   → Pay ~$0.85 to retrieve from vault (bridge out to L1)       │
...
|------|---------------|-------|
| Real BRC-20 on Bitcoin (visible on Unisat) | **~$0.85** | No way to reduce this |
...
- Dust limit (minimum UTXO): ~$0.50 (546 sats) ← **Fixed, cannot reduce**
- **Total: ~$0.85** (928 sats)
...
|----------|------|-------------|------------|
| **UniSat API** | ✅ Yes | **FREE** | **~$0.85** ⭐ |
| InscribeNFTs | ❌ UI only | 1,465 sats | ~$2.03 |
| Xverse | ⚠️ In dev | 2,000 sats | ~$2.62 |
| OrdinalsBot | ⚠️ Runes only | 2,000 sats | ~$2.70 |
| Gamma | ❌ UI only | 2,500 sats | ~$3.25 |
| UniSat UI | - | 3,150 sats | ~$3.74 |
...
| **BRC-20 transfer cost (L1)** | **~$0.85 per transfer** |
| **Highest cost component** | Dust limit (~$0.50, or 53% of total) |
...
|----------|---------------|----------------|--------------|----------------------|
| **L1 Direct** | ~$0.85 | ✅ Yes | ✅ Yes | Already on L1 |
| **Shadow Balance** | $0 | ❌ No | ❌ No (database) | Redeem: +$0.85 |
| **Merlin L2** | ~$0.05 | ❌ No (wrapped) | ❌ No (M-Token) | Bridge out: +$0.85 |
| **Spark (LRC-20)** | ~$0.01-0.05 | ❌ No | ❌ No (different token) | ❌ Cannot convert |

**Bottom Line**: The ~$0.85 L1 inscription cost is unavoidable if you want real, verifiable BRC-20 tokens on the Bitcoin mainnet.
...
1. You have a $1 bill (UTXO with 546+ sats)
...
- A UTXO with at least the **dust limit** (546 sats) = the minimum "bill size" recognized by the network
...
| **Inscribed UTXO** | Sats + witness data | Sat value + inscription value | 546 sats + BRC-20 inscription |
...
│   ┌─────────────────┐                                           │
│   │     $1 bill     │  You take a $1 bill (546+ sats UTXO)     │
│   │ ┌─────────────┐ │                                           │
...
│                                                                 │
│   THE BOTTOM LINE:                                              │
│   The $1 (546 sats) and the 10 BID are INSEPARABLE.            │
...
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   ✅ Lower dust limit (330 sats vs 546 sats)                    │
...
|--------------|------------|---------------------------|
| Legacy (P2PKH) | 546 sats | ~$0.50 |
| Native SegWit (P2WPKH) | 546 sats | ~$0.50 |
...
│   └── UTXOs smaller than 546 sats are too small to be worth    │
...
AT 1 SAT/VBYTE:
  ...
  └── Dust limit: ~$0.50 (FIXED, cannot be reduced) ← Largest cost!
```
...
│                                                                 │
│ OUTPUTS:                                                        │
│   └── New UTXO: 546 sats → recipient's Taproot address         │
...
| Dust Limit | Fixed (SegWit) | 546 | $0.50 | **59%** |
| **TOTAL** | | **928** | **$0.85** | 100% |
...
|----------|-------------|---------|------------|-------|--------------|
| 1 sat/vbyte | $0.17 | $0.18 | $0.50 | **$0.85** | Dust (59%) |
...
| Deploy BRC-20 token | ~$0.50-1.00 | One-time creation cost |
| **Transfer BRC-20 token** | **~$0.85** | Per transfer (2 TXs + dust) |
...
*Note: ~50% of the cost (~$0.50) is the dust limit, which is fixed regardless of fee rate. Costs vary with BTC price.*
...
│    • Dust: 546 sats locked in the UTXO                        │
└─────────────────────────────────────────────────────────────────┘
```
...
| Dust Limit (546 sats) | ~$0.50 | Beem | ✅ L1 |
| **TOTAL per user** | **~$1.00** | | |

**Key Insight**: At 1 sat/vbyte, the dust limit (~$0.50) represents 50% of the bridge-out cost.
...
| **Shared Cost** | Beem pays dust, user pays fees | ~$0.50 |
...
```
┌─────────────────────────────────────────────────────────────────┐
│     EVERY PATH TO REAL L1 BRC-20 COSTS ~$0.85                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   FROM SHADOW BALANCE:                                          │
│   ┌──────────────┐         ┌──────────────┐                    │
...
│   DIRECT L1:                                                    │
│   ┌──────────────┐         ┌──────────────┐                    │
│   │ Your L1      │ ──$0.85──▶ │ User's L1   │                    │
│   │ BRC-20       │ transfer │ BRC-20      │                    │
│   └──────────────┘         │ (on-chain)  │                    │
│                            └──────────────┘                    │
│                                                                 │
│   THE ~$0.85 L1 COST IS UNAVOIDABLE because:                   │
│   • Inscription TX (~$0.28) - write data to Bitcoin            │
│   • Send TX (~$0.17) - transfer UTXO to user                   │
│   • Dust limit (~$0.50) - minimum sats for UTXO                │
│                                                                 │
│   This is the cost of writing to the Bitcoin blockchain.        │
...
|----------|-----------------|-------------|-------------------------|----------------|----------|
| **BRC-20** | ~$0.85 | ✅ Witness | ✅ Unisat, OKX, OrdScan | L1 Ordinals indexers | ⭐⭐⭐⭐⭐ High |
| **BRC2.0** | ~$0.85 | ✅ Witness | ✅ Unisat + BRC2.0 indexer | L1 Ordinals + BRC2.0 indexers | ⭐⭐⭐ Growing |
...
| **ORC-20** | ~$0.85 | ✅ Witness | ⚠️ ORC-20 explorers | L1 ORC-20 indexers | ⭐⭐ Low |
...
| Pros | Cons |
|------|------|
| ✅ Highest adoption | ❌ ~$0.85/transfer @ 1 sat/vB (higher at congestion) |
...
| **Transfer cost @ 1 sat/vB** | ~$0.85 | ~$0.40 |
...
| **Bridge-out cost** | ~$0.85 (L1 inscription + send + dust) |
...
| Cheapest L1 fungible tokens | **Runes** | ~$0.40 vs BRC-20's ~$0.85 | ✅ Yes |
...
|----------|-----------------|-------|---------------------|-----------------------------|--------------------|
| **BRC-20** | ~$0.85 | L1 | ✅ Yes (witness) | ✅ Yes | ✅ Native |
| **BRC2.0** | ~$0.85 | L1 | ✅ Yes (witness) | ✅ Yes (+ BRC2.0 indexer) | ✅ Extension |
| **Runes** | ~$0.40 | L1 | ✅ Yes (OP_RETURN) | ✅ Yes (Runes explorers) | ❌ Different |
| **ORC-20** | ~$0.85 | L1 | ✅ Yes (witness) | ⚠️ ORC-20 explorers | ❌ Different |
...
- **Cheapest with eventual L1 visibility**: Merlin L2 (~$0.05 on L2, ~$0.85 to bridge out)
...
| 3 | Dust locked in UTXO | ~$0.50 | 🔴 **YES (53%)** |
| **TOTAL** | | **~$0.85** | |
...
| 5 | Dust locked in UTXO | ~$0.50 | 🔴 **YES (53%)** |
| **TOTAL** | | **~$0.85** | |
...
| 6 | Dust locked in UTXO | ~$0.50 | 🔴 **YES (53%)** |
| **TOTAL** | | **~$1.05** | |

### Key Insight (@ 1 sat/vbyte)

**At minimum fee rates, the dust limit (~$0.50) becomes the highest cost component (~53%).**
...
|----------|--------------|------------|----------------|--------------|
| Pure L1 | ~$850K | - | **~$850K** | **~$0.85** |
| Shadow → L1 | $0 | ~$850K | **~$850K** | **~$0.85** |
| L2 → L1 | $50K | ~$850K | **~$990K** | **~$0.99** |
...
|----------|-------------------|---------------|-------------------|
| Pure L1 | All users get L1 | N/A | **~$850K** |
...
│    ├── YES → Pure L1 (cost ~$0.85/user @ 1 sat/vB)             │
│    └── NO → Continue to Q2                                     │
│                                                                 │
...
│    └── Beem pays only dust (~$0.50) or nothing                 │
...
|----------|---------------------|-----------|-----------------|
| Pure L1, Beem pays | ~$850K | $0 | ✅ All users |
| Shadow + User-paid L1 | ~$0 | ~$0.85 at redemption | ✅ On demand |
| L2 + User-paid L1 | ~$50K | ~$0.85 at bridge-out | ✅ On demand |
...
3. **Offer L1 redemption** where **user pays the L1 fees** (~$0.85 at 1 sat/vbyte)
4. **Beem covers only dust** ($0.50/user) or passes that to user too

**Total Beem cost for 1M users: $0 - $50K** (vs ~$850K for pure L1 at 1 sat/vbyte)
...
┌─────────────────────────────────────────────────────────────────┐
│                                                                 │
│   🔴 TO GET REAL BRC-20 ON L1: ~$0.85 per user                 │
│                                                                 │
...
│   there is NO way to avoid the ~$0.85 L1 cost.                 │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| **Dust limit** | ~$0.50 | Same for all (unavoidable) |
...
|----------|-------------|-------------|------|----------|----------------|------|
| **UniSat API** | **FREE** | 182 sats | 546 sats | ~200 sats | **~$0.85** | ✅ Yes |
| **InscribeNFTs** | 1,465 sats | included | 546 sats | ~200 sats | **~$2.03** | ❌ UI only |
| **Xverse** | 2,000 sats | 117 sats | 546 sats | ~200 sats | **~$2.62** | ⚠️ In dev |
| **OrdinalsBot** | 2,000 sats | 199 sats | 546 sats | ~200 sats | **~$2.70** | ⚠️ Runes only |
| **Gamma** | 2,500 sats | 294 sats | 546 sats | ~200 sats | **~$3.25** | ❌ UI only |
| **UniSat UI** | 3,150 sats | 182 sats | 546 sats | ~200 sats | **~$3.74** | - |
...
| Inscription sats (dust) | 546 | ~$0.50 |
...
- **UniSat API is the clear winner** - FREE service fee → **~$0.85 total per transfer**
- Total = Inscription (182 sats network + 546 sats dust) + Send (~200 sats) = **928 sats**
- UI-based service fees range from **1,465 to 3,150 sats** (~$1.34-$2.89)
- UI services significantly increase total cost to ~$2.03-$3.74
...
**Bottom line**: UniSat API at $0.85/transfer is the most practical and cost-effective option.
...
2. **Total L1 cost at 1 sat/vbyte: ~$0.85 per user** (inscription + send + dust)
3. **At 1 sat/vbyte, dust limit (~$0.50) is 53% of total cost** - this is fixed and cannot be reduced
