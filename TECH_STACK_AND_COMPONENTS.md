...
**GPT context and memory — three sources:** For GPT calls we provide (1) **our own memory system** — long-term user preferences and facts stored in Pinecone and retrieved each turn; (2) **previous conversations** from the same chat — the backend sends **more than 5 turns** in `lastConversations`; we **limit to 5 turns** in cost-sensitive paths (e.g. base_gpt) to optimize for cost, while other nodes may use more (e.g. 15–20); and (3) a **`conversation_context`** field — an LLM-generated **summary** of the conversation thus far, produced during engagement/intent analysis, stored in Redis/Mongo, and passed downstream so prompts can use a concise summary as well as raw turns.
...
| **`lastConversations`** | Stored and passed by backend | Previous messages from the same chat (backend sends **more than 5**). We **limit to 5 turns** in cost-sensitive paths (e.g. base_gpt); engagement/context use more (e.g. 15–20). Newest first. |
...
1. Backend stores and passes previous conversations (**more than 5 turns**) in the request; we put them in `lastConversations`. In cost-sensitive paths we **limit to 5 turns** when building prompts to optimize for cost.
...
3. Nodes read context from `state["requestInfo"]`:
   ...
   - **engagement / context:** Use `lastConversations` (e.g. 15–20 turns, formatted) for engagement and intent prompts.
   - **base_gpt:** Uses **last 5 turns** from `lastConversations` plus `user_memory` (5-turn limit for cost optimization).
...
### 4.2 Past Conversations (lastConversations — Backend Sends More; We Limit to 5 for Cost)

**Conversational context:** The backend **sends more than 5 turns** of previous messages in `lastConversations`. We **limit to 5 turns** in cost-sensitive paths (e.g. base_gpt) to optimize for cost; other nodes (e.g. engagement, context) use more where needed. This is separate from long-term user memory (Section 4.3).

**Server-side persistence:**

| Mechanism | Where | Purpose |
|-----------|--------|--------|
| **MongoDB** | Collection from `AIGENT_CONVERSATION_COLLECTION` (e.g. `prompt_storing`) | `persist_user_state()` in `src/utils/state_persistence.py` upserts a state snapshot by `userId`, including conversation-related data. Backend stores and passes conversation context; we receive it in `lastConversations` and limit to 5 turns in cost-sensitive paths. |
...
- **GPT context and memory** = (1) **Our own memory system** — Pinecone-backed user sections, retrieved each turn and exposed as `requestInfo["user_memory"]`; (2) **Previous conversations** — backend sends **more than 5 turns** in `lastConversations`; we **limit to 5 turns** in cost-sensitive paths (e.g. base_gpt) to optimize for cost; (3) **`conversation_context`** — LLM-generated summary of the conversation thus far, stored in Redis/Mongo as `conversationContext`, passed via `engagement_info` to usecase and router, and appended to prompts as “Conversation Context: …”.
- **Current turn** = `latestMessage`; **conversational context** = `lastConversations` (we limit to 5 where optimizing for cost) + the summary field; used by base_gpt, engagement, router, and other nodes.
