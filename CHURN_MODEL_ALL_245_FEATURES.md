...
| `loan_paid_45` | In base pipeline: 1 if user's most recent loan in snapshot month was repaid within 45 days, 0 otherwise. (Computation may vary by enrichment script.) | **Repayment discipline.** Users who repay on time are financially stable and engaged. Churners rarely repay (98%+ have 0); non-churners average ~0.29. Low repayment indicates either financial distress (can't afford to stay) or disengagement (don't care to repay). Both paths lead to churn. |
| `nsf_count` | Count of NSF (non-sufficient funds) events in lookback (linefunds, status=failed, reason="insufficient funds") | **Payment friction and financial stress.** Repeated NSF events indicate the user is struggling to maintain sufficient balance. This correlates with involuntary churn (subscription payment fails) and voluntary churn (frustration with the product). Model shows churners have higher NSF counts on average. |
| `nsf_check` | 1 if no NSF events in lookback (nsf_count=0), 0 if any NSF (nsf_count>0). (In base pipeline: inverse of NSF presence, not "resolved".) | **No-NSF flag.** nsf_check=1 means the user had no NSF events — a positive signal. nsf_check=0 means they had at least one NSF; correlates with payment friction and churn risk. |
...
| `repayment_rate` | loan_paid_45 / max(loans_count, 1) | **Repayment discipline.** Normalizes loan_paid_45 by loan count. Low = poor repayment; high churn risk. (Exact interpretation depends on how loan_paid_45 is aggregated in the dataset.) |
...
| `mp_wt_unsub_hover_events` | WT unsubscription flow hover (events: WT current plan screen, selected reason, success screen) | **WT unsubscription intent.** User is exploring the WT-specific unsubscription flow. (WT = product/plan code; exact meaning not verified.) Captures a distinct cancel path from the main cancel flow. |
| `mp_wt_unsub_completion_events` | WT unsubscription completed (event: WT Subscription auto cancelled) | **WT unsubscription completed.** User completed the WT-specific unsubscription flow. Completion indicates strong churn intent. |
...
| `mp_wt_collection_failed_last_90d` | WT collection failed (event: WT collection failed) | **Payment friction.** WT collection failed; payment problem. |
| `mp_wt_collection_success_last_90d` | WT collection success (event: WT collection success) | **Recovery.** WT collection succeeded. |
